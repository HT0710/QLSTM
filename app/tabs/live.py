    ...
    def __call__(self):
        ...
        with gr.Row():
            ...
            with gr.Row(scale=1):
                ...
                future_slider = gr.Slider(
                    minimum=1, maximum=100, step=1, label="Future", interactive=True
                )
        ...
        with gr.Row():
            with gr.Row(scale=1):
                location_dropdown = gr.Dropdown(
                    choices=None, label="City", interactive=True
                )
            with gr.Row(scale=1):
                lat_dropdown = gr.Textbox(label="Latitude", interactive=True)
                lon_dropdown = gr.Textbox(label="Longitude", interactive=True)
